...
/*                                                DESCRIPTOPN
 * 
 * An application developed on top of the OpenVCIK Framework to control a wheelchair using the OpenVCI Kit.
 *
 * This application allows the speaker to:
 *
 * 1. utter individual commands and accordingly send motion commands to the wheelchair
 * 2. to RECORD-AND-PLAY the commands using the NEXT command
 * 3. to RETRACE the sequence hence bringing back the wheelchair to the original position. This action is errornous
 *     because of the heading control issues	
 *
 * This application also performs following functins:
 *
 * 1. collect compass sensor data for heading control
 * 2. obstacle avaoidance using the sonar data which it collects   
 *
 *
